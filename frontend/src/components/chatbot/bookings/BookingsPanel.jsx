...
const BookingsPanel = () => {
  ...
  const loadCalendar = async () => {
    try {
      setRefreshing(true);
      const response = await bookings.getCalendar(bot.id, year, month);
  ...
  const handleStatusUpdate = async (bookingId, newStatus) => {
    try {
      ...
      await bookings.updateStatus(bot.id, bookingId, newStatus);
      await loadCalendar();
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Manage reservations and appointments</p>
        </div>
        <button
          onClick={loadCalendar}
          disabled={refreshing}
          className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50 self-end"
        >
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <h4 className="font-semibold text-gray-900 flex items-center gap-2">
                        ...
                        {booking.guest_name || booking.customer_name || 'Unknown'}
                      </h4>
                    </div>
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    {(booking.phone || booking.customer_phone) && (
                      <div className="flex items-center gap-2">
                        ...
                        <a
                          href={getWhatsAppLink(booking.phone || booking.customer_phone)}
                          target="_blank"
                    ...
                    {(booking.people_count || booking.guests) && (
                      <div className="flex items-center gap-2">
                        <Users className="w-4 h-4 text-gray-400" />
                        <span>{booking.people_count || booking.guests} people</span>
                      </div>
                    )}
